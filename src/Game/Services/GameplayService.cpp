...
void GameplayService::LoadMap(int mapIndex) const {
    // Log the current working directory (where the program was started)
    std::filesystem::path cwd = std::filesystem::current_path();
    std::cout << "Current working directory: " << cwd << std::endl;

    // Convert the relative path to an absolute path
    std::filesystem::path filePath = dataPath;
    std::filesystem::path fullPath = std::filesystem::absolute(filePath);  // Convert to absolute path

    std::cout << "Attempting to open file at: " << fullPath << std::endl;

    // Open the file
    std::fstream fin;
    fin.open(fullPath, std::ios::in);
    if (!fin.is_open()) {
        std::cerr << "Error: file not found at path: " << fullPath << std::endl;
        return;
    }
    int roll;
